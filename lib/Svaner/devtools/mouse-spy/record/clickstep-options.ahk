 ...
 * @property {Map} config
 * @property {signal} curRecordMode
 * @property {signal} logMouseCoordMode
 * @property {signal} useRelative
...
MouseSpy_Record_ClickStepOptions(App, props) {
    config := props.config
    curRecordMode := props.curRecordMode
    logMouseCoordMode := props.logMouseCoordMode
    ...
    comp.render := this => this.Add(
        ...
        App.AddDDL("vstep-filler-ddl x+10 w145 Choose1", stepFillerTemplates).onChange(handleConfigTemplateUpdate),
        
        App.AddButton("x+5 w30 h20", "+").onClick((*) => MouseSpy_Record_StepFillerEditor("add", stepFillerTemplates)),
        App.AddButton("x+5 w30 h20", "✎").onClick((*) => MouseSpy_Record_StepFillerEditor("edit", stepFillerTemplates, { name: App["step-filler-ddl"].Text, snippet: App["step-filler-snippet"].Value})),
...
MouseSpy_Record_StepFillerEditor(mode, stepFillerTemplates, selectedTemplate := { name: "", snippet: "" }) {
    ...
    return (
        ...
        ; btns
        SFE.AddButton("x120 w80 h20", "&Save").onClick(handleTemplateUpdate),
        SFE.AddButton("x+10 w80 h20", "&Close").onClick((*) => SFE.Destroy()),
        SFE.AddButton("x+10 w20 h20 " . ((mode == "add" || stepFillerTemplates.value.Count == 1) && "Disabled") , "🗑")
