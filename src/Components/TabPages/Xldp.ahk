Xldp(QM, curSelectedScriptTab2, useDesktopXl) {
    ...
    return (
        ctrls := modules.map((module, index) => [
            QM.AddReactiveRadio(radioStyle(A_Index), module.description, curSelectedScriptTab2, ,
                ["Click", (r*) => handleSelect(ctrls, r[1], module)]),
            
            QM.AddReactiveEdit("x30 y+10 h25 w150  ReadOnly", "{1}", xlPath, A_Index),

            QM.AddReactiveButton("h25 w70 x+10", "选择文件", xlPath, ,
                ["Click", (*) => selectNewXl(index)]),
            
            QM.AddReactiveButton("h25 w70 x+10", "打开表格", xlPath, ,
